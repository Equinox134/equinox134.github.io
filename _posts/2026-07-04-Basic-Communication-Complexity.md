...
*The value of the protocol $`\mathcal{P}`$ on input $`(x,y)`$ is the label of the leaf reached by starting from the root and walking on the tree. At each internal node $`v`$ labeled by $`a_v`$, walk left if $`a_v(x)=0`$ and right if $`a_v(x)=1`$; at each internal node labeled by $`b_v`$, walk left if $`b_v(y)=0`$ and right if $`b_v(y)=1`$. The cost of the protocol $`\mathcal{P}`$ on input $`(x,y)`$ is the length of the path taken on input $`(x,y)`$. The cost of the protocol $`\mathcal{P}`$ is the height of the tree.*
...
If we have reached a node $v$ on the path which Alice speaks, then because she cannot distinguish between $(x_1, y_2)$ and $(x_1, y_1)$ (in both cases she evaluates $`a_v(x_1)`$) she will behave the same on both inputs. Thus on $(x_1, y_2)$ wwill also moe ve towards $\ell$. If we have reached a node $v$ in which Bob speaks, by the same reason Bob cannot distinguish between $(x_1, y_2)$ and $(x_2, y_2)$ so he will behave the same on both inputs, that is, again we will move towards $\ell$. $\blacksquare$
